...
#include "FreeDLiveLinkSourceEditor.h"
...
#define LOCTEXT_NAMESPACE "FreeDLiveLinkSourceEditor"

void FreeDLiveLinkSourceEditor::Construct(const FArguments& Args)
{
	...
	[
		...
		[
			...
			[
				...
				[
					SNew(STextBlock)
					.Text(LOCTEXT("FreeDPortNumber", "Port Number"))
				]
				+ SHorizontalBox::Slot()
				.HAlign(HAlign_Fill)
				.FillWidth(0.5f)
				[
					...
					.OnTextCommitted(this, &FreeDLiveLinkSourceEditor::OnEndpointChanged)
				]
			]
			+ SVerticalBox::Slot()
			.HAlign(HAlign_Right)
			...
			[
				SNew(SButton)
				.OnClicked(this, &FreeDLiveLinkSourceEditor::OnOkClicked)
				[
					SNew(STextBlock)
					.Text(LOCTEXT("Ok", "Ok"))
				]
...
void FreeDLiveLinkSourceEditor::OnEndpointChanged(const FText& NewValue, ETextCommit::Type)
{
...
FReply FreeDLiveLinkSourceEditor::OnOkClicked()
{
	TSharedPtr<SEditableTextBox> EditabledTextPin = EditabledText.Pin();
	if (EditabledTextPin.IsValid())
	{
